...
pub fn aencrypt(
    ...
    encrypt_with_randomness(pk, &m_normal, &r)
}
...
pub(crate) fn derive_randomness(
    ...
    let dk_bytes = dk.dk.to_bytes_be();
    ...
    let r = BigUint::from_bytes_be(&hash_bytes) % q;
...
mod tests {
    ...
    fn test_aencrypt_xor_normal_decrypt() {
        let (pk, sk, dk) = akeygen(64).expect("akeygen");
        let (ct, _covert_enc) = aencrypt_xor(&pk, &dk, b"hi", b"covert payload")
            .expect("aencrypt_xor");
    ...
    fn test_aencrypt_xor_covert_roundtrip() {
        ...
        let (ct, covert_encrypted) = aencrypt_xor(&pk, &dk, b"hi", covert_msg)
            .expect("aencrypt_xor");
